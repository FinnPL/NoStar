...
graph TB
    subgraph "Vieta Site"
        V_Default[Default VLAN<br/>192.168.178.x]
        V_Athena[Athena VLAN<br/>10.10.0.x]
        Pi4[Pi4<br/>Docker Services]
        Apollo[Apollo NAS]
        V_Athena --- Pi4
        V_Athena --- Apollo
    end
    
    subgraph "Minerva Site"
        M_Default[Default VLAN<br/>192.168.179.x]
        M_Athena[Athena VLAN<br/>10.0.0.x]
        Pi3[Pi3<br/>Monitoring]
        Zeus[Zeus NAS]
        M_Athena --- Pi3
        M_Athena --- Zeus
    end
    
    V_Athena -.->|VPN Tunnel| M_Athena
    Apollo -.->|Sync| Zeus
    V_Default -.->|Traefik Only| V_Athena
    M_Default -.->|Traefik Only| M_Athena
```
